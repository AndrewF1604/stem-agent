...
### 3.7 Adversarial guard test

§3.5 showed the guard fires on *organic* proposer mutations. That left open: would the guard generalize to adversarial input it didn't organically encounter? I hand-crafted 8 degenerate genomes (empty prompt, contradictory instructions, refusal prompt, lorem ipsum, force-number, anti-search, max_steps=1, unconditional reflection, "be over-confident" prompt) and ran each through the same fitness + canary gate as if it were proposed.

Baseline: G0 val accuracy = 0.60. Canary set = 6 questions G0 solved.

| Adversarial label          | Val acc | Pass fitness (> 0.60)? | Got to canary? |
| -------------------------- | :-----: | :--------------------: | :------------: |
| empty_prompt               | 0.20    | no                     | —              |
| lorem_ipsum                | 0.60    | no (tied)              | —              |
| force_number               | 0.00    | no                     | —              |
| refusal                    | 0.00    | no                     | —              |
| anti_search                | 0.20    | no                     | —              |
| max_steps_1                | 0.60    | no (tied)              | —              |
| unconditional_reflection   | 0.50    | no                     | —              |
| overconfidence_prompt      | 0.60    | no (tied)              | —              |

**Zero of 8 passed the fitness gate**; three tied G0 exactly (0.60). The guard never fired because no adversarial cleared the prior check. This is partly the expected outcome (the fitness gate is supposed to be strict) and partly a methodological reveal: with n=10 val and a `>` comparison, an adversarial would need to *strictly* beat 0.60 while regressing canary, which is hard to engineer because canary is a subset of val — a mutation that breaks canary questions necessarily *lowers* val unless it also gains on non-canary questions.

The most informative single result is **lorem_ipsum tying G0 at 0.60**. A nonsense Latin prompt with no instructions about the task scored the same as the real instruction prompt. The model effectively *ignored* the system prompt and answered from the question text + web_search results. Three other adversarials (force_number, refusal) followed the bad instructions and scored 0; two more (anti_search, empty_prompt) scored 0.20 (some recall, no retrieval). So the model's prompt-following is *selective*: when the adversarial prompt is task-contrary, the model follows it and fails; when the adversarial is task-irrelevant, the model ignores it and falls back on the question + retrieval pipeline. This is a real mechanical finding I didn't have before: in this setup, system-prompt fidelity is a small fraction of the agent's behavior compared to the question+retrieval signal.

The honest read on the guard: under adversarial input *generated by hand*, the fitness check did all the gating work. The guard's load-bearing role appeared only in §3.5 with an *organic* proposer mutation that gained on some val questions while losing on others. That asymmetry (gain on uncovered questions, loss on covered ones) is hard to construct manually but the proposer produced it naturally. With more time I'd construct an adversarial that's engineered to pass fitness by exactly +1 question while regressing canary by ≥1 — that would directly test the guard on adversarial input.
...
- **Why prompt-following is selective, not absolute.** The §3.7 adversarial test produced a single striking result: a *lorem ipsum* prompt (Latin filler, no task instruction) scored the same val accuracy as the real prompt. When the adversarial prompt was *task-contrary* ("answer with a number", "refuse to answer"), the model followed it and scored zero. When the adversarial prompt was *task-irrelevant* (lorem ipsum, empty), the model ignored it and fell back on the question text + web_search retrieval. Mechanically: attention attends to whatever is most predictive of the next token. Task-relevant instructions are weighted; task-irrelevant text is essentially ignored when there's a clear, well-structured user message + retrieved evidence to focus on instead. This places an upper bound on how much prompt-engineering can change behavior in this setup, and explains why `REFINE_PROMPT` mutations rarely moved the needle in §3.4.
...
5. **Engineer a *fitness-passing canary-breaking* adversarial.** §3.7 ran 8 hand-crafted bad genomes through the gate; all 8 were caught by fitness before reaching canary. The guard's discriminating role only showed up on the organic proposer mutation in §3.5. A targeted adversarial that gains exactly +1 question on non-canary val while losing ≥1 canary question would put the guard mechanism under direct test. This requires per-question crafting and access to the val composition.
